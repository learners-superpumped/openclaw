...
let deps: VncProxyDeps;
let wss: WebSocketServer;

export function initVncProxy(d: VncProxyDeps): void {
  deps = d;
...
async function handleConnection(clientWs: WebSocket, req: IncomingMessage): Promise<void> {
  // 1. Parse instanceId from URL: /instances/:instanceId/vnc
  const url = new URL(req.url || "", `http://${req.headers.host}`);
  const match = url.pathname.match(/^\/instances\/([^/]+)\/vnc$/);
  if (!match) {
  ...
  const managerWsUrl =
    `${managerWsBase}/api/instances/${instance.instanceId}/vnc` +
  ...
  managerWs.on("open", () => {
    ...
    // Ping both sides to keep connections alive (LB idle timeout)
