...
export default class RFB extends EventTargetMixin {
  ...
  sendKey(keysym, code, down) {
    ...
    if (this._qemuExtKeyEventSupported && scancode) {
      ...
      Log.Info(
        "Sending key (" + (down ? "down" : "up") + "): keysym " + keysym + ", scancode " + scancode,
      );
  ...
  _updateConnectionState(state) {
    ...
    switch (state) {
      ...
      case "disconnected":
        if (oldstate !== "disconnecting") {
          Log.Error(
            "Bad transition to disconnected state, " + "previous connection state: " + oldstate,
          );
          return;
        }
        break;
      ...
      case "connecting":
        if (oldstate !== "") {
          Log.Error(
            "Bad transition to connecting state, " + "previous connection state: " + oldstate,
          );
          return;
        }
        break;
    ...
    switch (state) {
      ...
      case "disconnected":
        this.dispatchEvent(
          new CustomEvent("disconnect", { detail: { clean: this._rfbCleanDisconnect } }),
        );
        break;
    }
  }
  ...
  _setCapability(cap, val) {
    ...
    this.dispatchEvent(
      new CustomEvent("capabilities", { detail: { capabilities: this._capabilities } }),
    );
  }
  ...
  _handleSecurityReason() {
    ...
    if (reason !== "") {
      ...
      return this._fail(
        "Security negotiation failed on " + this._securityContext + " (reason: " + reason + ")",
      );
    } else {
      this.dispatchEvent(
        new CustomEvent("securityfailure", { detail: { status: this._securityStatus } }),
      );
  ...
  _negotiatePlainAuth() {
    if (
      ...
      this.dispatchEvent(
        new CustomEvent("credentialsrequired", { detail: { types: ["username", "password"] } }),
      );
      return false;
    }
  ...
  _negotiateStdVNCAuth() {
    ...
    if (this._rfbCredentials.password === undefined) {
      this.dispatchEvent(
        new CustomEvent("credentialsrequired", { detail: { types: ["password"] } }),
      );
      return false;
    }
  ...
  _negotiateARDAuth() {
    ...
    if (
      this._rfbCredentials.ardPublicKey != undefined &&
    ...
    let clientKey = legacyCrypto.generateKey({ name: "DH", g: generator, p: prime }, false, [
      "deriveBits",
    ]);
  ...
  async _negotiateARDAuthAsync(keyLength, serverPublicKey, clientKey) {
    ...
    const cipher = await legacyCrypto.importKey("raw", key, { name: "AES-ECB" }, false, [
      "encrypt",
    ]);
  ...
  static genDES(password, challenge) {
    ...
    const key = legacyCrypto.importKey("raw", passwordChars, { name: "DES-ECB" }, false, [
      "encrypt",
    ]);
...
RFB.cursors = {
  ...
  dot: {
    ...
    rgbaPixels: new Uint8Array([
      255, 255, 255, 255, 0, 0, 0, 255, 255, 255, 255, 255, 0, 0, 0, 255, 0, 0, 0, 0, 0, 0, 0, 255,
