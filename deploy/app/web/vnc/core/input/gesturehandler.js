...
export default class GestureHandler {
  ...
  _twoTouchTimeout() {
    ...
    let deltaTouchDistance = Math.abs(
      Math.hypot(avgD.first.x, avgD.first.y) - Math.hypot(avgD.last.x, avgD.last.y),
    );
  ...
  _getPosition() {
    ...
    return { first: { x: fx / size, y: fy / size }, last: { x: lx / size, y: ly / size } };
  }
  ...
  _getAverageMovement() {
    ...
    return { x: totalH / size, y: totalV / size };
  }
  ...
  _getAverageDistance() {
    ...
    return { first: { x: fdx, y: fdy }, last: { x: ldx, y: ldy } };
  }
}
