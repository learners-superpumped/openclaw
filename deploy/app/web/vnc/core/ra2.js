...
class RA2Cipher {
  ...
  async setKey(key) {
    this._cipher = await legacyCrypto.importKey("raw", key, { name: "AES-EAX" }, false, [
      "encrypt, decrypt",
    ]);
  }
...
export default class RSAAESAuthenticationState extends EventTargetMixin {
  ...
  _waitCredentialsAsync(subtype) {
    const hasCredentials = () => {
      if (
        subtype === 1 &&
        this._getCredentials().username !== undefined &&
  ...
  async negotiateRA2neAuthAsync() {
    ...
    const clientRSACipher = (
      await legacyCrypto.generateKey(
        {
          name: "RSA-PKCS1-v1_5",
